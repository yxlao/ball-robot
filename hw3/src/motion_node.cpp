...
class MotionNode {
  ...
  cv::Mat prevgray, gray, flow, cflow, fgMaskMOG2, prevFrame, currFrame,
  frameDiff, thresholdFrame, diff_frame;
  bool first;
 ...
 public:
  MotionNode(std::string name) :
    ...
    image_sub_ = it_.subscribe("/usb_cam/image_raw", 1, &MotionNode::frameCallback,
                               this);
  ...
  void frameCallback(const sensor_msgs::ImageConstPtr& msg) {
    ...
    cv::Mat element;
    cv::Mat img_smooth, img_color, img_gray, img_result, img_edge_color,
    img_edge_gray_x, img_edge_gray_y;
    ...
    try {
      ...
      case VISUALIZATION_BOUNDING_BOX:
        ...
        element = cv::getStructuringElement(erosion_type, cv::Size(2 * erosion_size + 1,
        ...
        */
        ...
        cv::findContours( img_gray, contours, hierarchy, CV_RETR_TREE,
  ...
  void executeCB(const ChangeVideoModeGoalConstPtr &goal) {
    ...
    ROS_INFO("Executing, changing Video Mode to %d, Visualization to %d.",
             goal->video_mode_id, goal->visualization_mode_id);
